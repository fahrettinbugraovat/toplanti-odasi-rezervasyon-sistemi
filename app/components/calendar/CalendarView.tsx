...
// Tüm sistemdeki standart saat aralıkları
...
export default function CalendarView() {
  ...
  // Takvime özel seçim stateleri (Oda ID'sine göre izole edildi)
  const [localSelection, setLocalSelection] = useState<{room: Room, slots: string[]} | null>(null);
  const [localTitle, setLocalTitle] = useState('');

  // Güvenli Tarih Formatlayıcıları
  ...
  // İptal edilen veya onay bekleyen kayıtlar takvimde dolu olarak gösterilmez
  const activeOperations = operations.filter((op: any) => op.status !== 'iptal' && op.status !== 'bekliyor');

  // Geçmiş saat kontrolü (Dilimin bitiş saati baz alınır)
  ...
  // DÜZELTİLDİ: Odayı, Tarihi ve Saati %100 İzole Eden Dinamik Hesaplayıcı
  const getReservationSpan = (roomName: string, slot: string, currentDateFormatted: string, todayFormatted: string) => {
    for (const op of activeOperations) {
      // 1. Tarih eşleşmesi
      const isTodayMatch = (op.date === 'Bugün' && currentDateFormatted === todayFormatted);
      if (op.date !== currentDateFormatted && !isTodayMatch) continue;

      // 2. Oda ismi eşleşmesi (Kesin ve izole)
      ...
      if (slotStartHour === opStartHour) {
        ...
        return { op, span, isStart: true };
      } else if (slotStartHour > opStartHour && slotStartHour < opEndHour) {
        return { op, span: 0, isStart: false };
      }
    }
    return null;
  };
  ...
  const selectEmptySlot = (room: Room, slot: string) => {
    // Oda ID'sine göre katı izolasyon
    ...
    const newSlots = isSelected 
      ? localSelection.slots.filter(s => s !== slot)
      : [...localSelection.slots, slot];
  ...
  const confirmReservation = () => {
    ...
    try {
      ...
      // Benzersiz ID ile merkezi havuza ekleme
      setOperations([{
          ...
          <div className="grid overflow-x-auto w-full" style={{ gridTemplateColumns: `180px repeat(${TIME_SLOTS.length}, minmax(110px, 1fr))` }}>
            ...
            {rooms.map((room) => (
              <div key={room.id} className="contents group">
                ...
                {TIME_SLOTS.map((slot) => {
                  // DÜZELTİLDİ: React Key çakışmasını engellemek için room.id ile slot birleştirildi
