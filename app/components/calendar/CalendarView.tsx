...
export default function CalendarView() {
  ...
  // YENİ: Toplantının saatinin geçip geçmediğini kontrol eden motor eklendi
  const isMeetingCompleted = (op: any) => {
    if (!currentTime) return false;
    const currentYear = new Date(currentTime).getFullYear();
    const todayStrLocal = getLocalYYYYMMDD(new Date(currentTime));
    
    let parsedDate = op.date;
    if (parsedDate === 'Bugün') parsedDate = todayStrLocal;
    else if (parsedDate === 'Yarın') {
       const tmr = new Date(currentTime); tmr.setDate(tmr.getDate() + 1);
    ...
    // Aynı günse bitiş saatini kontrol et
  ...
  return (
    <section className="h-[calc(100vh-4.5rem)] md:h-[calc(100vh-5rem)] overflow-hidden flex flex-col bg-[#f3f6ff] p-4 text-[#101b35] dark:bg-[#141414] dark:text-[#e5e7eb] md:p-5">
      <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-4 xl:flex-row flex-1 min-h-0 overflow-hidden">
        
        {/* TAKVİM GRID ALANI */}
        <div className="w-full xl:flex-1 flex flex-col border border-gray-200 bg-white dark:border-[#2d2d2d] dark:bg-[#1c1c1c] rounded-lg shadow-sm overflow-hidden h-full">
          
          {/* TAKVİM HEADER KISMI */}
          <div className="relative shrink-0 flex h-14 items-center justify-between border-b border-gray-200 bg-gray-50 px-5 dark:border-[#2d2d2d] dark:bg-[#212121]">
          ...
          {/* İÇ SCROLL ALANI - Sayfa sabit durur, sadece bu tablo alanı aşağı/yukarı ve sağa/sola kayar */}
          <div className="flex-1 min-h-0 overflow-auto relative bg-white dark:bg-[#1c1c1c]">
            <div className="grid min-w-max w-full" style={{ gridTemplateColumns: `180px repeat(${TIME_SLOTS.length}, minmax(110px, 1fr))` }}>
              
              {/* SAATLER (Sticky Top - Üste Yapışır) */}
              <div className="sticky top-0 left-0 z-30 h-12 border-b border-r border-gray-200 px-4 flex items-center font-bold text-xs text-gray-600 dark:text-gray-400 dark:border-[#2d2d2d] bg-gray-50 dark:bg-[#1a1a1a]">Oda</div>
              {TIME_SLOTS.map((slot) => (
                <div key={`header-${slot}`} className="sticky top-0 z-20 h-12 border-b border-r border-gray-200 px-2 flex items-center justify-center font-bold text-[11px] text-gray-600 dark:text-gray-400 dark:border-[#2d2d2d] bg-gray-50 dark:bg-[#1a1a1a]">
                  {slot}
                </div>
              ))}
              ...
              {rooms.map((room) => (
                <div key={room.id} className="contents group">
                  ...
                  {/* ODA İSMİ (Sticky Left - Sola Yapışır) */}
                  <div className="sticky left-0 z-10 h-[88px] border-b border-r border-gray-200 px-4 py-4 dark:border-[#2d2d2d] bg-white dark:bg-[#1c1c1c] flex flex-col justify-center transition-colors group-hover:bg-gray-50 dark:group-hover:bg-[#252525]">
        ...
        {/* SAĞ TARAF: REZERVASYON ÖZET PANELİ */}
        {hasSelectedRange && localSelection && (
          <aside className="flex h-full overflow-y-auto w-full flex-col border border-gray-200 bg-white rounded-lg shadow-sm p-5 dark:border-[#2d2d2d] dark:bg-[#1c1c1c] xl:w-[280px] shrink-0">
